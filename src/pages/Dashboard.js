...
const Dashboard = ({ user, onLogout }) => {
  ...
  try {
    setLoading(true);
    const teacherQuizzes = await quizService.getQuizzesByTeacher(currentUser.uid);
    
    console.log('Loaded quizzes:', teacherQuizzes); // Simple debug
    ...
    setStats({
      ...
      activeQuizzes: teacherQuizzes.filter(q => q.isActive).length,
      totalParticipants: teacherQuizzes.reduce((sum, quiz) => sum + (quiz.participants || 0), 0)
    });
  } catch (error) {
